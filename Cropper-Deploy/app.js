...
function updateCropUI(animate = false) {
  const computed = getComputedStyle(cropBox);
  const previous = { left: computed.left, top: computed.top, width: computed.width, height: computed.height };
  cropBox.getAnimations().forEach((animation) => animation.cancel());
  const b = getCanvasBounds(), c = state.crop; const x = b.x + b.w * c.x, y = b.y + b.h * c.y, w = b.w * c.w, h = b.h * c.h;
  const next = { left: `${x}px`, top: `${y}px`, width: `${w}px`, height: `${h}px` };
  Object.assign(cropBox.style, next);
  if (animate && !window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
    cropBox.animate([previous, next], { duration: 600, easing: 'ease-in-out' });
  }
...
// Keep short taps visible for the full Figma transition; selected options stay active.
const touchLayoutMotionQuery = window.matchMedia('(max-width: 1024px)');
const reducedMotionQuery = window.matchMedia('(prefers-reduced-motion: reduce)');
document.querySelectorAll('.mobile-option, .mobile-action, .tablet-option, .tablet-action').forEach((button) => {
  let feedbackTimer;
  button.addEventListener('click', () => {
    if (!touchLayoutMotionQuery.matches || reducedMotionQuery.matches || button.disabled) return;
    clearTimeout(feedbackTimer);
    button.classList.add('is-animating');
    feedbackTimer = setTimeout(() => button.classList.remove('is-animating'), 650);
  });
});
